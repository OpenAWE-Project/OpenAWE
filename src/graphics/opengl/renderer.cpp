...
Renderer::Renderer(Platform::Window &window, const std::string &shaderDirectory) : Graphics::Renderer(window.getSize()),
	_window(window),
...
void Renderer::setRenderPlane(const glm::vec2 size) {
	...
	//
	_depthTexture = std::make_unique<Texture>(size.x, size.y, "depth_buffer");
	_normalTexture = std::make_unique<Texture>(size.x, size.y, "normal_buffer");
	_depthstencilBuffer = std::make_unique<Renderbuffer>(size.x, size.y, GL_DEPTH24_STENCIL8, "depthstencil_renderbuffer");
