...
void main() {
    ...
    vec2 vNormalDx = texture(g_sNormalMap, pass_UV).wy * 2.0 - 1.0;
    float fNormalZ = sqrt(1.0 - dot(vNormalDx.x, vNormalDx.y));

    vec3 vBitangent = cross(pass_Normal, pass_Tangent);
    vec3 vNormal =
        mat3(pass_Tangent, pass_Bitangent, normalize(pass_Normal.xyz)) *
        vec3(vNormalDx, fNormalZ);
#else // PROP_NORMAL_MAP
    vec3 vNormal = pass_Normal;
#endif // PROP_NORMAL_MAP

    out_Normal.xyz = vNormal * 0.5 + 0.5;
    out_Normal.w = 1.0;
}
